...
class OrderController extends Controller
{
    ...
    {
        $carts=\app\models\Cart::find()->where(['user_id'=>\Yii::$app->user->identity->id_user])->all();
        foreach ($carts as $cart)
        {
            $order = new Order();
            $order->user_id=\Yii::$app->user->identity->id_user;
            $order->product_id=$cart->product_id;
            $order->count=$cart->count;
            $order->save(false);
            $cart->delete();
        }
        return $this->redirect(['/order/index?OrderSearch[user_id]=' . \Yii::$app->user->identity->id_user]);
    }
